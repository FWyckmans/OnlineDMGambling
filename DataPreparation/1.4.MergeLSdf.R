...
dQ <- cbind(dLS1, dLS3[3:length(dLS3)])
...
##### Recruitement
dRecr <- read_excel("dRecrutementGamblingDaw.xlsx")
dRecr <- AddDummyCol(dRecr, "AUDIT")

for (i in c(1:length(dRecr$AUDIT))){
  if (dRecr$Mail1[i] %in% dQ$Mail1){
    dRecr$AUDIT[i] = dQ$AUDIT[dQ$Mail1==dRecr$Mail1[i]]
}}

write_xlsx(dRecr, "dRecrutementGamblingDaw.xlsx")
