...
write.table(dTotM, paste0(Output_path, "Anonyme/dTotMerged.txt"), col.names = T, row.names = F, sep = "\t", dec = ".")

##### Remove participant who failed Daw Task
dTotOK <- dTot%>%
  filter(OKRL_AfterGamb == 1)%>%
  filter(OKRL_AfterNeutr == 1)

##### Export
write.table(dTotOK, paste0(Output_path, "Anonyme/dTotAnonyme.txt"), col.names = T, row.names = F, sep = "\t", dec = ".")
write_xlsx(dTotOK, paste0(Output_path, "Anonyme/dTotAnonyme.xlsx"))

# dTot anonymous
dTotA <- dTotOK[,-2]

write.table(dTotA, paste0(Output_path, "Processed/dTot.txt"), col.names = T, row.names = F, sep = "\t", dec = ".")
write_xlsx(dTotA, paste0(Output_path, "Processed/dTot.xlsx"))
